...
type EndStateSubscriber = (
  finalState: State,
) => void;

export class WriterlyDocumentWalkerV2 {
  walkSubscribers: WalkSubscriber[] = [];
  endStateSubscribers: EndStateSubscriber[] = [];

  public registerWalkSubscriber(subscriber: WalkSubscriber) {
    this.walkSubscribers.push(subscriber);
  }

  public registerEndStateSubscriber(subscriber: EndStateSubscriber) {
    this.endStateSubscribers.push(subscriber);
  }
  ...
  private walk(
    ...
    for (let lineNumber = 0; lineNumber < document.lineCount; lineNumber++) {
      ...
      let lineType = WriterlyDocumentWalkerV2.updateState(state, lineNumber, indent, content);

      for (const subscriber of this.walkSubscribers) {
        subscriber(prevState, lineType, state, lineNumber, indent, content);
      }
    }

    for (const subscriber of this.endStateSubscribers) {
      subscriber(state);
    }
  }
